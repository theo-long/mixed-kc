    ...
    def wrapper(self, env, state, *args, **kwargs):
        ...
        if isinstance(res, _bdd.Function):
            ...
            def add_node_rec(u, visited):
                ...
                visited.add(nid)
                if nid == 1:
                    tracker.all_nodes_info[nid] = {"id": nid, "var": "True" if u.node > 0 else "False", "low": None, "high": None}
                    return
            ...
            live_node_ids = list(set([abs(obj.node) for obj in gc.get_objects() if isinstance(obj, _bdd.Function)]))
            ...
            res_str = f"EnumResult(bits={[abs(b.node) for b in getattr(res, 'bits', [])]})"
...
def hooked_model_count(bdd, u, count, weights):
    ...
    tracker.wmc_steps.append({
        "node_idx": abs(u.node) if hasattr(u, "node") else str(u),
        "result": str(res)
    })
    return res
...
def extract_ast_tree(node):
    ...
    if isinstance(node, kc.terms.Let):
        return {"id": nid, "type": "Let", "var": node.var, "binding": extract_ast_tree(node.binding), "body": extract_ast_tree(node.body)}
    elif isinstance(node, kc.terms.IfThenElse):
        return {"id": nid, "type": "IfThenElse", "cond": extract_ast_tree(node.cond), "then_expr": extract_ast_tree(node.then_expr), "else_expr": extract_ast_tree(node.else_expr)}
    elif isinstance(node, kc.terms.Flip):
        prob = extract_ast_tree(node.prob) if getattr(node, "prob", None) and isinstance(node.prob, kc.base.PExpr) else {"type": "primitive", "val": node.prob}
        ...
        return {"id": nid, "type": "Eq", "left": extract_ast_tree(node.left), "right": extract_ast_tree(node.right)}
    else:
